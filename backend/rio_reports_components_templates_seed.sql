...
-- componentes a embarcaciones existentes por nombre.
--
-- Notas:
-- - Las cantidades por motor no existen en el modelo actual; se guarda item/ref.
-- - Las asignaciones a embarcacion solo se aplican si el barco ya existe.
...
BEGIN
    ...
    WHERE type = TRIM(p_type)
      ...
      AND LOWER(COALESCE(manufacturer, '')) = LOWER(COALESCE(TRIM(p_manufacturer), ''))
...
BEGIN
    ...
    IF v_vessel_id IS NULL THEN
        ...
        RAISE NOTICE 'Embarcacion creada para Guardia Civil: %', p_vessel_name;
    ELSE
        UPDATE vessels
...
BEGIN
    ...
    -- Asignaciones a barcos existentes. Si el barco no existe, no hace nada.
...
-- WHERE LOWER(v.name) IN ('rio saja', 'rio irati', 'rio iro', 'rio ter')
